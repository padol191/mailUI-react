...
const doLogin = function (username, password) {
  if (
    ...
    this.state.userCredenentials[username] === password
  ) {
    ...
    this.setState({
      ...this.state,
      loginError: "Unknown username/password combination",
    });
  }
};
...
const toggleComposeEmail = function () {
  this.setState({
    ...this.state,
    isComposingEmail: !this.state.isComposingEmail,
  });
};
...
const sendNewMail = function (to, cc, subject, body) {
  ...
  const newEmail = {
    from: {
      ...
      name: users[this.state.loggedInUser],
    },
...
const doLogout = function () {
  this.setState({
    ...this.state,
    loggedInUser: null,
    loggedInUserData: initialState.loggedInUserData,
  });
